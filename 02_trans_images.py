...
#data augmentation
#train data - Resize and Rotate/Flip
#test_data - Resize only
...
for i in os.listdir(src_path):
	...
	train_fruit_path = os.path.join(train_path, str(i)) 
	test_fruit_path = os.path.join(test_path, str(i)) 
	
	if os.path.isdir(src_fruit_path):
		util.checkdir(dest_fruit_path)	
		util.checkdir(train_fruit_path)	
		util.checkdir(test_fruit_path)	
		...
		for f_img in os.listdir(src_fruit_path):
			...
			try:
				...
				im_train_path = os.path.join(train_fruit_path, str(im_name) + '_' )
				im_test_path = os.path.join(test_fruit_path, str(im_name) + '_' )
				
				#resize
				im_resize = im.resize((resize_width, resize_height), Image.ANTIALIAS)
				save_jpg(im_resize, im_dest_path + '_resized')
				save_jpg(im_resize, im_test_path + '_resized')
				
				#flip
				im_flip = im_resize.transpose(Image.FLIP_LEFT_RIGHT)
				save_jpg(im_flip, im_dest_path + '_flip')
				save_jpg(im_flip, im_train_path + '_flip')
				...
				for j in range(len(size)):
					...
					save_jpg(imRotate, im_train_path + str(j) +  '_rotate')
					save_jpg(imRotate2, im_train_path + '_' + str(j+len(size)) + '_rotate')
